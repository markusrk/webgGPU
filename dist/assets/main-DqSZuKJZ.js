import { p as pr, h as hr, g as gr, y as yr, c as cr, d as dr, i as ie, E as Er, r as rayIntersectsRectangle, m as mr } from "./intersect-tLLRUPzs.js";
...
const isPointInsidePolygon = pr(`(point, polygon2, polygonLength) => {
  const x = point[0];
  const y = point[1];
  ...
  for (let i of ti.range(numVertices)) {
    let x1 = polygon2[i][0];
    let y1 = polygon2[i][1];
    let x2 = polygon2[i + 1][0];
    let y2 = polygon2[i + 1][1];
...
function isPointInsidePolygonJS(point, polygon2) {
  const x = point[0];
  const y = point[1];
  const numVertices = polygon2.length - 1;
  let crossings = 0;
...
const rayTrace = async (polygonInJS2, windowsInJS) => {
  ...
  const polygon2 = hr.field(2, gr, [polygonLength]);
  polygon2.fromArray(polygonInJS2);
...
const generateWindowsAlongWall = (polygon2, options) => {
  ...
  for (let i = 0; i < polygon2.length - 1; i++) {
    const startPosition = polygon2[i];
    const endPosition = polygon2[i + 1];
